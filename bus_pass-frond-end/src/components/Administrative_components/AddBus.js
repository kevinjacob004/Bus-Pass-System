...
const AddBus = () => {
  ...
  useEffect(() => {
    fetchRoutes(); // Fetch routes when the component mounts
  }, []);
  ...
  // Handle route selection (store ID instead of name)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert("Bus creation failed: " + result.message);
      }
    } catch (error) {
    ...
    <div className="font-sans bg-gray-50 min-h-screen flex flex-col">
      ...
      <main className="flex-grow flex items-center justify-center p-6">
        <div className="bg-white shadow-lg rounded-xl p-8 w-full max-w-md border border-gray-200">
          <h2 className="text-3xl font-bold text-gray-800 text-center mb-6">CREATE A NEW BUS Details</h2>
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div>
              <label className="block text-gray-700 font-medium mb-1">Bus Number</label>
              <input type="number" name="busNo" value={formData.busNo} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" />
            </div>
            <div>
              <label className="block text-gray-700 font-medium mb-1">Bus Registration Number</label>
              <input type="text" name="busRegistrationNo" value={formData.busRegistrationNo} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" />
            </div>
            <div>
              <label className="block text-gray-700 font-medium mb-1">Max SeatCapacity</label>
              <input type="number" name="maxSeatCapacity" value={formData.maxSeatCapacity} onChange={handleChange} className="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm" />
            </div>
            <div>
            ...
            <button type="submit" className="w-full bg-blue-600 text-white py-3 rounded-lg hover:bg-blue-700 transition duration-300 shadow-md text-lg font-semibold">Confirm</button>
          </form>
        </div>
      </main>
